...
if (isset($_GET['id'])) {
    ...
    $sql = $conn1->query("SELECT * FROM item_products WHERE id = $product_id");
    if ($sql) {
        $row = $sql->fetch_assoc();

        $db_product = $row['product_services'];
        $db_producttype = $row['id_product'];
        $db_datecreated = $row['date_created'];
        $db_datemodified = $row['date_modified'];
    }
}
...
if (isset($_POST['submit'])) {
    ...
    $query = $conn1->query("UPDATE item_products SET productservices = '$productitem', id_product = '$producttype', date_created = 'DC', date_modified =  '$DM' WHERE id=$product_id");

    if ($query) {
        $_SESSION['message'] = 'Product succesfully updated';
                    ...
                    <div class="col-lg-6">
                        <form action="" method="post">
                            <div class="form-group">
                                <label>Product</label>
                                <input type="text" name="productitem" class="form-control" value="<?php echo $db_product ?>">
                            </div>
                            <div class="form-group">
                                <label>Type</label>
                                <input type="text" name="producttype" class="form-control" value="<?php echo $db_producttype ?>">
                            </div>
                            <div class="form-group">
                                <label>Date Created</label>
                                <input type="text" name="DC" class="form-control" value="<?php echo $db_datecreated ?>">
                            </div>
                            <div class="form-group">
                                <label>Date Modified</label>
                                <input type="text" name="DM" class="form-control" value="<?php echo $db_datemodified?>">
                            </div>
                            <div class="form-group">
                                <input type="submit" name="submit" class="btn btn-primary" value="Send">
                                <button type="back" name="back" class="btn btn-primary" onClick="javascript:window.location.href='product_add.php'; return false">Back</button>
                            </div>
                        </form>
                    </div>
                </div>
